    ...
    <div className='max-w-4xl mx-auto'>
      <div className='text-[14px] prose prose-slate dark:prose-invert max-w-none'>
        ...
        components={{
          ...
          ul({children}){
            return <ul className='list-decimal pl-6 space-y-2 my-4'>{children}</ul>
          },
          ol({children}){
    ...
    <div className='relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200 '>
      <div className='flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200'>
        ...
        <button onClick={copyCode} className='text-gray-500 hover:text-gray-700 focus:outline-none relative group' aria-label='Copy code'>
          {copied ? (<LuCheck size={16} className='text-green-600'/>) : (<LuCopy size={16} className='absolute -top-8 right-0 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition'/>)}
